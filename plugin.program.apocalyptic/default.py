...
def maintMenu(view=None):
	...
	addFile('Show [B]ALL[/B] [I][COLOR %s]([/COLOR]Apocalyptic[COLOR %s])[/COLOR][/I]  Submenus: %s' % (COLOR1, COLOR1, maint.replace('true',on).replace('false',off)), 'togglesetting', 'showmaint', icon=ICONMAINT, themeit=THEME2)
...
def net_tools(view=None):
	...
	setView('files', 'viewType')
def speedMenu():
	xbmc.executebuiltin('Runscript("special://home/addons/plugin.program.Apocalyptic/speedtest.py")')
######################################################################################################################
######################################################################################################################		
